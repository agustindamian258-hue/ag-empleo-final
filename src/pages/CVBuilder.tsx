    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-gray-100 px-5 py-4 shadow-sm">
        ...
        <p className="text-gray-400 text-xs">
          Completá los datos y descargá tu CV en PDF
        </p>
      </header>
      ...
      <div className="px-4 pt-5 space-y-3">
        ...
        {(
          [
            { name: 'summary',    placeholder: 'Resumen profesional (2-3 líneas)',                                          rows: 3 },
            { name: 'skills',     placeholder: 'Habilidades (separadas por coma)\nEj: Excel, Atención al cliente, Manejo de caja', rows: 2 },
            { name: 'experience', placeholder: 'Experiencia (una por línea)\nEj: 2022-2024 — Vendedor en Empresa X',        rows: 4 },
            { name: 'education',  placeholder: 'Educación\nEj: Bachiller — Colegio Nacional (2020)',                        rows: 2 },
          ] as const
        ).map((f) => (
          <textarea
            key={f.name}
        ...
        >
            ...
            <div>
              <div className="bg-blue-700 px-6 py-5 text-white">
                ...
                <p className="text-blue-200 text-xs mt-0.5">
                  {[form.email, form.phone, form.ciudad].filter(Boolean).join(' · ')}
                </p>
              </div>
              <div className="p-5 space-y-4">
                ...
                {expList.length > 0 && (
                  <div>
                    ...
                    {expList.map((e) => (
                      <p key={e} className="text-sm text-gray-700 py-1 border-l-2 border-blue-200 pl-3 mb-1">
                        {e}
                      </p>
                    ))}
                  </div>
          ...
          {template === '2' && (
            <div className="flex">
              <div className="w-2/5 bg-purple-700 p-4 text-white min-h-64">
                ...
                {skillsList.length > 0 && (
                  <div className="mt-4">
                    <p className="text-[9px] font-black uppercase tracking-widest text-purple-300 mb-2">
                      Skills
                    </p>
                    {skillsList.map((s) => (
                      <p key={s} className="text-[11px] py-0.5 border-b border-purple-600">{s}</p>
                    ))}
                  </div>
                )}
              </div>
              <div className="w-3/5 p-4 space-y-3">
                ...
                {expList.length > 0 && (
                  <div>
                    <p className="text-[9px] font-black text-purple-700 uppercase tracking-widest mb-1">
                      Experiencia
                    </p>
                    {expList.map((e) => (
                      <p key={e} className="text-xs text-gray-700 mb-1">{e}</p>
                    ))}
                  </div>
                )}
                {form.education && (
            ...
            <div className="p-6 font-serif">
              ...
              {expList.length > 0 && (
                <div className="mb-4">
                  ...
                  {expList.map((e) => (
                    <p key={e} className="text-sm text-gray-700 mb-1">— {e}</p>
                  ))}
                </div>
              )}
              {form.education && (
          ...
          {template === '4' && (
            <div className="p-5 font-mono text-xs">
              ...
              {form.summary && (
                <div className="mb-3">
                  <p className="font-black uppercase text-gray-800 border-b border-gray-300 mb-1">
                    PERFIL
                  </p>
              ...
              {expList.length > 0 && (
                <div className="mb-3">
                  ...
                  {expList.map((e) => (
                    <p key={e} className="text-gray-700 mb-0.5">• {e}</p>
                  ))}
                </div>
              )}
              {form.education && (
