...
interface Message { id: string; senderId: string; text: string; createdAt: FirestoreTimestamp | null; }
interface OtherUser { name: string; photo: string; }
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const unsubChat = onSnapshot(chatRef, async (snap) => {
      if (snap.exists()) {
        ...
        } catch (e) { console.error('[Chat] fetchUser', e); }
      }
    });
    ...
    try {
      ...
      if (!chatSnap.exists()) {
        ...
        await setDoc(chatRef, {
          ...
          lastMessage: msg, lastMessageAt: serverTimestamp(), unreadBy: [otherUid],
        });
      } else {
        await updateDoc(chatRef, { lastMessage: msg, lastMessageAt: serverTimestamp(), unreadBy: [otherUid] });
      }
      await addDoc(collection(db, 'chats', chatId, 'messages'), {
        senderId: me.uid, text: msg, createdAt: serverTimestamp(),
      });
    } catch (e) {
  ...
  const avatarUrl = otherUser.photo ||
    ...
    <div className="flex flex-col bg-gray-50 dark:bg-gray-950" style={{ height: '100dvh' }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-gray-400 dark:text-gray-500 text-sm font-bold">Empezá la conversación con {otherUser.name}</p>
          </div>
        )}
        {messages.map((msg) => {
      ...
      <div className="shrink-0 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 px-3 py-3 flex items-end gap-2">
        <button
          ...
          className="shrink-0 w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center active:scale-90 transition-transform"
        >
