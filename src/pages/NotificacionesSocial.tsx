// src/pages/NotificacionesSocial.tsx
...
import {
  BellIcon, HeartIcon, UserIcon,
  CheckIcon, TrashIcon, ChatBubbleLeftIcon,
  StarIcon,
} from '@heroicons/react/24/outline';

interface Notif {
  id:       string;
  tipo:     'like' | 'seguidor' | 'comentario' | 'reaccion' | 'mensaje';
  titulo:   string;
  mensaje:  string;
  leida:    boolean;
...
export default function NotificacionesSocial() {
  const { user }  = useTheme();
      ...
      <div className="max-w-lg mx-auto px-4 pt-6">
          ...
          <div className="flex items-center gap-2">
            {sinLeer > 0 && !modoSelec && (
              <button onClick={marcarTodas}
                className="flex items-center gap-1 text-xs font-bold text-purple-600 active:opacity-60">
        ...
        {cargando ? (
          ...
          <div className="flex flex-col items-center gap-3 pt-20 text-center">
            ...
            <p className="text-sm text-gray-300 dark:text-gray-600">Acá aparecerán tus likes, comentarios y más</p>
          </div>
        ) : (
          <div className="space-y-2">
            {notifs.map(n => {
              const seleccionada = seleccion.has(n.id);
              const icono = ICONO[n.tipo] ?? ICONO.like;
              const bg    = BG[n.tipo]    ?? BG.like;
              return (
                <button
                  key={n.id}
                  onClick={() => handleTap(n)}
                ...
                >
                  {modoSelec && (
                    <div className={`w-5 h-5 rounded-full border-2 shrink-0 mt-0.5 flex items-center justify-center ${
                      seleccionada ? 'bg-purple-500 border-purple-500' : 'border-gray-300 dark:border-gray-600'
                    }`}>
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    ...
                    {!n.leida && <span className="w-2 h-2 rounded-full bg-purple-500" />}
                  </div>
                </button>
              );
            })}
